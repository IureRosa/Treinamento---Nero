...
while (toc(t) <= 2*tmax)
    ...
    if toc(tc) > 0.1
        ...
        rx = 1.5;
        ry = 1;
        P.pPos.Xd(1) = rx*sin(2*pi*toc(t)/tmax);
        P.pPos.Xd(2) = ry*sin(4*pi*toc(t)/tmax);

        P.pPos.Xd(7) = 2*pi/tmax*rx*cos(2*pi*toc(t)/tmax);
        P.pPos.Xd(8) = 4*pi/tmax*ry*cos(4*pi*toc(t)/tmax);
        ...
        if toc(tp) > 0.1
            ...
            h(1) = plot(rx*sin(2*pi*[-pi:pi/180:pi]),ry*sin(4*pi*[-pi:pi/180:pi]),'r--');
